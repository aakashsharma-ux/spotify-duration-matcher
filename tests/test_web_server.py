...
class TestMissingReport:
    ...
    def test_report_missing_excludes_matched_files_on_first_analyse(self, client, auth_headers):
        """Regression test: right after the FIRST analyse (no reassign/rename
        in between), files that DID match a sheet row must never appear in
        the 'not on sheet' section. The 'used' flag on session audio_files
        is not set by the initial analyse pass, so this section must be
        derived fresh from match_table on every request, not from 'used'.
        """
        with tempfile.TemporaryDirectory() as tmp:
            good1 = Path(tmp) / "Stone In My Shoe - Animal Logic.mp3"
            good2 = Path(tmp) / "Flying High - Army Of Lovers.mp3"
            extra = Path(tmp) / "Totally Unrelated File - Nobody.mp3"
            for p in (good1, good2, extra):
            ...
            r = client.get("/api/report-missing", headers=auth_headers)
        content = r.data.decode("utf-8")
        not_on_sheet_section = content.split("SONGS UPLOADED BUT NOT ON SHEET")[1]
        assert "Totally Unrelated File" in not_on_sheet_section
        assert "Stone In My Shoe" not in not_on_sheet_section
