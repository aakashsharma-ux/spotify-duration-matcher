...
def create_app(config: dict) -> Flask:
    ...
    def download_missing_report(sess: dict) -> Any:
        ...
        # Re-sync 'used' from the CURRENT match_table before deriving
        # unmatched_audio — match_table may have changed since the initial
        # analyse (reassign, assign-unmatched, rename), and 'used' is never
        # set by the initial analyse pass itself, so it can't be trusted
        # on its own here.
        matched_paths = {r["matched_file"] for r in match_table if r.get("matched_file")}
        for af in audio_files:
            af["used"] = af["path"] in matched_paths
